...
{
    public partial class UCStepTagTable : XtraUserControl
    {
        ...
        public void ShowTable()
        {
            ...
            {
                //jjk, 22.06.07 - LS Timelog 업데이트
                if (((CProfilerProject_V8)m_cProject).PLCMaker == UDM.DDEACommon.EMPlcMaker.LS)
                    UpdateLSLogCount(CLogHelper.LogHistory.LsTimeLogS, true);
                else
        ...
        //jjk, 22.05.27 - LS S접점 
        private void UpdateLSLogCount(CTimeLogS cTimeLogS, bool bInitNeed)
        {
            ...
            if (bInitNeed)
            {
                foreach (CTag ctag in m_cProject.TagS.Values)
                {
                    ...
                    if (((CProfilerProject_V8)m_cProject).PLCMaker == UDM.DDEACommon.EMPlcMaker.LS && Utils.GetAddressHeader(ctag.Address).Equals("S"))
                    {
                        List<CTimeLog> findStypeTimeLogS = cTimeLogS.FindAll(x => x.LsStypeAddress.Equals(ctag.LSMonitoringAddress));
                        if (findStypeTimeLogS.Count == 0)
                            continue;
                ...
                for (int index1 = 0; index1 < lstLogS.Count; index1++)
                {
                    CTimeLogS tempTimeLogS = lstLogS[index1];
                    for (int index2 = 0; index2 < tempTimeLogS.Count; index2++)
                    {
                        string sKey = tempTimeLogS[index2].LsStypeAddress;
                        var findTagS = m_cProject.TagS.Values.ToList().FindAll(x => x.LSMonitoringAddress == sKey);
                        foreach (CTag oriTag in findTagS)
                        {
                            ++oriTag.LogCount;
                        }
                    }
        ...
        private void ShowTable(CProfilerProject cProject)
        {
            ...
            ////yjk, 22.04.14 - S접점의 Step정보를 전체  Step 리스트에 추가 
