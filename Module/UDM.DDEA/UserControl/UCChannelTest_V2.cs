...
namespace UDM.DDEA
{
    public partial class UCChannelTest_V2 : XtraUserControl
    {
        #region Variables
        ...
        //jjk, 20.03.24 - TCP dll 통신이 되는지 확인하기 위하여 Test 모드 생성 //true 테스트 모드 on / false 테스트 off
        private bool m_bModbusDllTestMode = false;


        #endregion

        public event UEventHandlerConnect UEventConnect;
        //jjk, 20.02.26 - CGroupIOT 이벤트 연결 
        public event UEventHandlerTagValueChanged UEventTagValueChanged;

        #endregion

        public UCChannelTest_V2()
        {
            InitializeComponent();
        ...
        public CDDEAProject_V6 Project
        {
        ...
        public void SetStopTestConfig()
        {
            //jjk, 20.03.20 - 연결 테스트 종료
            tmrChannelTest.Stop();
            if (DisConnection())
            {
        ...
        private bool Connect()
        {
            ...
            m_cGroupIot = new CDDEAGroup_IOT(m_cProject);
            m_cGroupIot.UCChannelTest = this;
            ...
            if (m_cIotConfigMS != null)
            {
                ...
                else if (m_emCategory == EMCommunicationCategory.OPC || m_emCategory == EMCommunicationCategory.Modbus)
                {
                    m_cChannelS.Clear();
                    //jjk, 20.03.24 - Modbus Test Mod 분기
                    if (m_bModbusDllTestMode)
                    {
                        //CModbusTcpConnectorTest.ModbusTcpMasterRead();
                        CModbusTcpConnectorTest.ModbusRtuMasterRead();
                    }
                    else
                    {
                        m_bConnect = m_cGroupIot.IotConnection();
                    }
                }
            }
            else
            {
                //jjk, 20.02.12 - 새로 저장 안하고 이전 버전 파일일때도 테스트가 진행이 되어야됨
                m_bConnect = Connection(m_emMaker);
            }
        ...
        private bool DisConnection()
        {
            ...
            else if (m_emCategory == EMCommunicationCategory.OPC)
            {
                m_cGroupIot.IotConnection();
                m_cGroupIot.StopIotMonitor();
            }
            else if (m_emCategory == EMCommunicationCategory.Modbus)
            {
                //jjk, 20.03.24 - Modbus Test Mod 분기
                if (m_bModbusDllTestMode)
                {
        ...
        private bool Connection(EMPlcMaker emMaker)
        {
            ...
            if (m_cConfig.SelectedItem == EMConnectTypeMS.None)
                return false;
        ...
        private void UCChannelTest_Load(object sender, EventArgs e)
        {
        }
        ...
        private void btnStartChannelTest_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                MessageBox.Show("연결 성공.\r\n통신설정 저장 하시려면 적용 버튼을 누르십시오.", "통신연결", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
            }
        }
